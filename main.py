...
from superDatabase import *
from superTools import draw_triangle
import os
import pygame
import sys
...
def draw_player(screen, pos, color, number=None, show_numbers=False):
    pygame.draw.circle(screen, color, pos, 12)  # Slightly larger players for higher resolution
    if show_numbers and number is not None:
        font = pygame.font.Font(None, 24)  # Larger font for higher resolution
...
def get_clicked_player(pos, team):
    for i, player_pos in enumerate(team):
        ...
        if distance < 12:  # Adjusted click detection radius
            return i
    return None

def is_ball_clicked(pos):
    distance = ((pos[0] - BALL_POS[0])**2 + (pos[1] - BALL_POS[1])**2)**0.5
    return distance < 10  # Slightly larger ball click detection
